...
function Dianaday(objDate) {
   var i, leap=0, temp=0;
   var baseDate = new Date(1900,0,31);// 1900年1月31日 是 庚子年正月初一
   var offset   = (objDate - baseDate)/86400000; //86400000ms为1天，计算当前日期与1900年1月31日相隔的天数
   this.dayCyl = offset+40;
   this.monCyl = 14;
   for(i=1900; i<2050 && offset>0; i++) {
      temp = lYearDays(i); //农历i年的天数
      offset -= temp;
      this.monCyl += 12;
   }
   if(offset<0) {
   ...
   for(i=1; i<13 && offset>0; i++) {
      if(leap>0 && i==(leap+1) && this.isLeap==false){	//闰月
          --i; this.isLeap = true; temp = leapDays(this.year);}//leapDays返回农历y年闰月的天数
      else{
         temp = monthDays(this.year, i);}//monthDays返回农历y年m月的总天数
      if(this.isLeap==true && i==(leap+1)) this.isLeap = false;	//解除闰月
      offset -= temp;
      if(this.isLeap == false) this.monCyl++;//相隔的月份
   }
...
function calendar(y,m) {
   ...
   if ((m+1)==5){fat=sDObj.getDay()}//为了计算母亲节
   if ((m+1)==6){mat=sDObj.getDay()}//为了计算父亲节
   for(var i=0;i<this.length;i++) {//遍历公历每一天
      if(lD>lX) {//初始ld>lx，执行一次后当前日期<当月总天数；随着ld++，ld>lx,进入下一个月
         ...
         if (lM==12){eve=lX} //最后一个月，将腊月的天数赋给eve，为了后面判断除夕
		 if(n==0) firstLM = lM;//第一个月
         lDPOS[n++] = i-lD+1;//lDPOS[0] = 0
      }
